...
import taskcoachlib.patterns.observer as observer
import weakref
...
class ObservableComposite(Composite):
    ...
    def modificationEventTypes(class_):
        try:
            ...
            eventTypes = []
        return eventTypes + [class_.addChildEventType(), class_.removeChildEventType()]
...
class CompositeCollection(object):
    ...
    def extend(self, composites, event=None):
        ...
        super(CompositeCollection, self).extend(compositesAndAllChildren, event=event)
    ...
    def remove(self, composite, event=None):
        return (
            self.removeItems([composite], event=event) if composite in self else event
        )
